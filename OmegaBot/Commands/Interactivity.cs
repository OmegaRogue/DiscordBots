...
namespace OmegaBot.Commands
{
	...
	internal class Interactivity
	{
		...
		{
			var msg = await ctx.RespondAsync((await Aesthetics.ContructLineAsync(22,22)).FormatCode());
			for (int i = 0; i < 10; i++)
			{
				for (int j = 0; j < 20; j++)
				{
					await msg.DeleteAsync();
					msg = await dep.Client.SendMessageAsync(ctx.Channel,(await Aesthetics.ContructLineAsync(j, i)).FormatCode());
		...
		{
			await ctx.RespondAsync(await ctx.RawArgumentString.ToBlockTextAsync());
