...
namespace OmegaBot.Commands
{
	...
	internal class Interactivity : BaseCommandModule
	{
		...
		public Interactivity(InteractivityExtension interactiviti, TenorClient tenorClient)
		{
		...
		public TenorClient TenorClient { get; }
		...
		[Command("waitforreact")]
		[Description("Waits for a reaction.")]
		public async Task WaitForReaction(CommandContext ctx)
		{
			...
			// wait for a reaction
			var em = await interactivity.WaitForReactionAsync(xe => xe.Emoji.Equals(emoji), ctx.User,
				TimeSpan.FromSeconds(60));
			if (em.Equals(null))
			{
				// quote
				var embed = new DiscordEmbedBuilder
				{
					...
					{
						Name = em.Result.Message.Author is DiscordMember mx
							? mx.DisplayName
							: em.Result.Message.Author.Username,
		...
		#region waitfortyping

		[Command("waitfortyping")]
		...
		public async Task SendPaginated(CommandContext ctx)
		{
			...
			var lipsumPages = interactivity.GeneratePagesInEmbed(lipsum);

			// send the paginator
			await interactivity.SendPaginatedMessageAsync(ctx.Channel, ctx.User, lipsumPages, new PaginationEmojis(),
				PaginationBehaviour.Default, PaginationDeletion.DeleteMessage, TimeSpan.FromMinutes(5));
		}

		#endregion

		[Command("toblock")]
		public async Task ToBlockAsync(CommandContext ctx)
		{
			await ctx.RespondAsync(ctx.RawArgumentString.ToBlockText());
		...
		[Command("vote")]
		[Description("Run a poll with Upvote and Downvote.")]
		public async Task VoteAsync(CommandContext ctx, [Description("How long should the poll last.")]
			TimeSpan duration, [Description("What the poll is about.")]
			params string[] topicc)
		{
			var topic = string.Join(' ', topicc);
			// first retrieve the interactivity module from the client
			var interactivity = ctx.Client.GetInteractivity();

			var upVote = DiscordEmoji.FromName(ctx.Client, ":thumbsup:");
			var downVote = DiscordEmoji.FromName(ctx.Client, ":thumbsdown:");
			...
			var embed = new DiscordEmbedBuilder
			{
				...
				Description = "Vote about " + topic + "for " + duration +
				              "\n\n:thumbsup:: yee\n\n:thumbsdown:: REEEE\n\nPowered By Tenor"
			};
			var msg = await ctx.RespondAsync(embed: embed);
		...
		public async Task TestAsync(CommandContext ctx)
		{
			var x = 10;
			var y = 5;
			var check = DiscordEmoji.FromName(ctx.Client, ":Grid10x10:");
			var up = DiscordEmoji.FromName(ctx.Client, ":arrow_up_small:");
			var down = DiscordEmoji.FromName(ctx.Client, ":arrow_down_small:");
			var left = DiscordEmoji.FromName(ctx.Client, ":arrow_backward:");
			var right = DiscordEmoji.FromName(ctx.Client, ":arrow_forward:");
			var interactivity = ctx.Client.GetInteractivity();


			var msg = await ctx.RespondAsync(x + ";" + y + "  \n" + Aesthetics.ContructLine(x, y).FormatCode());

			await msg.CreateReactionAsync(check);
		...
		{
			var msg = await ctx.RespondAsync(Aesthetics.ContructLine(22, 22).FormatCode());
			for (var i = 0; i < 10; i++)
			for (var j = 0; j < 20; j++)
			{
				await msg.DeleteAsync();
				msg = await ctx.Client.SendMessageAsync(ctx.Channel, Aesthetics.ContructLine(j, i).FormatCode());
			}
		}
