...
namespace OmegaBot.Commands
{
	...
	internal class Interactivity
	{
		...
		public async Task SendPaginated(CommandContext ctx)
		{
			...
			// send the paginator
			await interactivity.SendPaginatedMessageAsync(ctx.Channel, ctx.User, lipsum_pages, new PaginationEmojis(ctx.Client), PaginationBehaviour.Default, PaginationDeletion.DeleteMessage, TimeSpan.FromMinutes(5));
		}
