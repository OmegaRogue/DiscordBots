	...
	internal class Interactivity : BaseCommandModule
	{
			...
			TimeSpan duration, [Description("What the poll is about.")] params string[] topicc)
		{
			...
			var embed = new DiscordEmbedBuilder
			{
				...
				Description = "Vote about " + topic + "for " + duration.ToString() + "\n\n:thumbsup:: yee\n\n:thumbsdown:: REEEE"
			};
			var msg = await ctx.RespondAsync(embed: embed);
		...
		public async Task WaitForReaction(CommandContext ctx)
		{
			...
			// wait for a reaction
			var em = await interactivity.WaitForReactionAsync(xe => xe.Emoji.Equals(emoji), ctx.User, TimeSpan.FromSeconds(60));
			if (em.Equals(null))
			{
				// quote
				var embed = new DiscordEmbedBuilder
		...
		public async Task SendPaginated(CommandContext ctx)
		{
			...
			// send the paginator
			await interactivity.SendPaginatedMessageAsync(ctx.Channel, ctx.User, lipsumPages, new PaginationEmojis(ctx.Client), PaginationBehaviour.Default, PaginationDeletion.DeleteMessage, TimeSpan.FromMinutes(5));
		}
